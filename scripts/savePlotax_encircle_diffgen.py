    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            for i in range(aa.shape[1]):
                genid = sampled_gen_idxs[i]
                g_sample.append(genid)
                ...
                for c_i,c in enumerate(color):
                    ...
                    one_gen = pred_list[genid==c_i]
                    for g_i in range(np.sum(genid==c_i)):
                        # if not plot_all:
                        #     if traplot[j] == 0:
                        #         continue
                        out_a=one_gen[g_i,:,i,:]
                        ...
                        x1=bb[:,0]
                        y1=bb[:,1]
                        ax.plot(x1, y1, '*',markersize=5,color=c)
                    ...
                    savePath = os.path.join('plot','mgplot_pdf_20')
                    if not os.path.exists(savePath):
                        os.makedirs(savePath)
                    name = tyID[i][0]['new'][1]+str(tyID[i][0]['new'][0])
                    plt.savefig(os.path.join(savePath,name+'_'+c+'_'+str(np.sum(genid==c_i)/genid.shape[0])+'.pdf'))
                    # plt.show()
                    plt.close()
        ...
        np.save('plot_data_evngen_20.npy', plot_dic)
